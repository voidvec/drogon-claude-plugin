...
> 本文档规划了 drogon-claude-plugin 升级至 v0.2.0 的迭代实施步骤、排期和验收用例。本实施计划基于 [PRD/upgrade_review.md](upgrade_review.md) 的评审意见制定。
...
1.  **修改 [CLAUDE.md](../CLAUDE.md)**：
    *   在现有 A–L 组末尾，追加新增的 M–V 组规则，并融入 [upgrade_review.md](upgrade_review.md) 中的补充规范（协程传值、Advice 回调规范、RequestStream 正确 API）。
    *   修订 A.6、B、C.2、D.1、K.3、E.4 等现有条目，完成双向引用与语义补正。
2.  **修改 [posttooluse.py](../hooks/posttooluse.py) —— 修复 v0.1.0 遗留缺陷**：
...
3.  **升级配置文件与基础文档**：
    ...
    *   修改 [README.md](../README.md) 中关于技能数和钩子数的统计信息。

### 阶段二：新增 8 个技能的创建与验证 [优先级：中 (P1)]
1.  **创建以下技能子目录**，每个目录包含 `SKILL.md` 与 `references/code-guide.md`：
    *   [drogon-gen-session-auth](../skills/drogon-gen-session-auth)
    *   [drogon-gen-file-upload](../skills/drogon-gen-file-upload)
    *   [drogon-gen-advice](../skills/drogon-gen-advice)
    *   [drogon-gen-coroutine-handler](../skills/drogon-gen-coroutine-handler)
    *   [drogon-gen-http-client](../skills/drogon-gen-http-client)
    *   [drogon-gen-rate-limiter](../skills/drogon-gen-rate-limiter)
    *   [drogon-gen-monitoring](../skills/drogon-gen-monitoring)
    *   [drogon-gen-lambda-handler](../skills/drogon-gen-lambda-handler)
