...
> 本文件由 `drogon` 插件提供（Claude Code / ZCode 双宿主）。规则针对 drogon v1.9.13。
>
...
- **Claude Code**：`claude plugin marketplace add https://github.com/voidvec/drogon-claude-plugin` → `claude plugin install drogon` → 升级 `claude plugin update drogon`。
- **ZCode**：在插件管理中添加同一 marketplace（GitHub 源）后安装 `drogon`。
- **CLI 安装器**（PyPI/npm，`drogon-claude-plugin` 命令）：`install` / `verify` / `upgrade` / `uninstall`，资产装入项目 `.drogon-plugin/`，不触碰项目自有文件。
